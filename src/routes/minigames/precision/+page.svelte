<script lang="ts">
	...
	function handleAnswer(value: number) {
		...
		if (isCorrect) {
			...
			streak++;
			game.addEnergy(5);
			if (streak % 15 === 0 && hearts < MAX_HEARTS) hearts++;
		} else {
	...
	>
		...
		<div class="flex items-center gap-1">
			{#each Array(MAX_HEARTS) as _, i (i)}
				<span class="text-base {i < hearts ? '' : 'opacity-40 grayscale'}">❤️</span>
			{/each}
		</div>
	</header>
	...
	<div class="relative z-10 flex w-full flex-1 flex-col items-center justify-center px-6 py-6">
		{#if showResult}
			<div class="flex flex-col items-center gap-6 text-center">
				<div class="text-5xl">{gameOver ? '💔' : '🎯'}</div>
				<h2 class="text-2xl font-bold text-white">
					{gameOver ? '¡Sin corazones!' : '¡Completado!'}
				</h2>
				...
				<p class="text-sm text-gray-500">
					{correctCount >= 45
						...
						: correctCount >= 35
							? '¡Muy bien! Tu puntería es excelente.'
							: 'Seguí practicando para mejorar tu precisión.'}
				</p>
		...
		{:else if currentQ}
			...
			<div class="mb-8 w-full max-w-lg">
				<div class="h-2 w-full overflow-hidden rounded-full bg-[#1E1E2F]">
					<div
						class="h-full rounded-full bg-red-400 transition-all duration-300"
